...
function start(w, options, cb) {
  ...
  cb(document.body.innerHTML, images);
}
...
exports.parse = function parse(theHtml, url, options, callback) {
  ...
  try {
    ...
    start(win, options, function(html, images) {
      //dbg(html);
      var time = new Date().getTime() - startTime;
      callback({title: document.title, content: html, time: time / 1000, inputLength: theHtml.length, images: images});
    });
  } catch(e) {
    //throw e;
